...
class Application:
    def __init__(self, master=None):
        ...
        self.btcadastro = Button(self.container8, text="Confirmar Cadastro", font=self.fonte, width=16)
        self.btcadastro["command"]=self.cadastroUsuario
        self.btcadastro.pack(side=LEFT)

        self.btlogin = Button(self.container9, text="Login", font=self.fonte, width=16)
        self.btlogin["command"]=self.loginUsuario
        self.btlogin.pack(side=LEFT) 
    ...
    def loginUsuario(self):
        email = self.txtemail_login.get()
        senha = self.txtsenha_login.get()

        self.cursor.execute("SELECT * FROM cliente WHERE email=%s AND senha=%s", (email, senha))
